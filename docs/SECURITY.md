...
Database migrations are the source of truth for grants, RLS, transactional workflow enforcement, audit immutability, and view safety. The current local evidence includes a clean native Supabase reset, seven pgTAP files with 226 passing assertions, 13 authenticated portal workflows, and the two design-preview Playwright tests. These results are local evidence only; hosted settings and manual release observations remain separate gates.
...
- Security-sensitive functions set a controlled `search_path`, re-evaluate current membership/role/date status, lock mutable request rows, and expose only required execution grants. Authenticated callers have access only to the read-only private predicates needed to evaluate policies/views. The reviewer-name RPC returns requested/actual display names only when the caller can view that request, without granting direct access to the underlying reviewer profile or membership.
...
| ------- | ------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| SEC-002 | Medium        | The one-time bootstrap is serialized and service-role-only. Native pgTAP passed the allowed first call, audit creation, non-service-role denial, and second-call denial, but a real dual-control first-admin execution and successor-admin workflow have not been observed.                | `supabase/migrations/20260829030000_initial_nhs_backend.sql`, `supabase/tests/004_bootstrap.sql`, `src/lib/supabase/admin.ts`, `docs/OPERATIONS.md`, `docs/QA.md` | **Partially mitigated; operational deployment gate.** Execute the real first-admin bootstrap with dual control, verify its audit, then provision and test a successor teacher administrator through the protected application workflow. |
...
- **Resolved SEC-001 (local runtime assurance):** a clean native Supabase reset and all seven pgTAP files (226 assertions) passed. The 13 authenticated portal workflows also passed, including a true simultaneous two-browser-context review race that produced one success, one conflict, and exactly one persisted approval. The Playwright configuration rejects non-loopback application and Supabase URLs before these destructive synthetic flows run. Hosted configuration and smoke testing remain under SEC-005 (`supabase/tests`, `tests/e2e/portal.spec.ts`, `playwright.config.ts`, `docs/QA.md`).
...
- **Resolved SEC-007 (source/unit/native database assurance):** invitation delivery now prepares without mutating send facts, calls Auth first, and acknowledges only provider acceptance with an idempotent UUID carried as non-secret Auth metadata for reconciliation. Provider failure never acknowledges; a lost acknowledgement response retries with the same UUID without resending. The coordinator's five unit tests and the native pgTAP invitation-integrity coverage passed. Actual hosted email-provider acceptance, rejection, and resend behavior remain an external gate under SEC-005 (`src/lib/auth/invitation-delivery.ts`, `src/lib/auth/invitation-delivery.test.ts`, `src/app/actions/admin-actions.ts`, `supabase/tests/006_invitation_send_integrity.sql`, `docs/QA.md`).
