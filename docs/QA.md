...
This comparison supports the intended hierarchy, palette, density, responsive transformation, and primary interaction anatomy. It is not a pixel-diff pass. Authenticated data states, browser E2E, 200% zoom, other browser engines, dialogs, full keyboard order, contrast below axe's serious threshold, and screen-reader announcements remain release checks.
...
| --------------------------------- | ----------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `pnpm install --frozen-lockfile`  | Not recorded                        | Run against the final lockfile                                                                                                                                                                                                                                             |
...
| `pnpm test`                       | Passed, 2026-08-29                  | Vitest: 11 files, 199 tests passed; includes 8 domain files/181 tests, 9 safe-navigation tests, 4 password-context tests, and 5 invitation-delivery tests                                                                                                                  |
...
| Migration/seed PGlite check       | Passed (supplemental), 2026-08-29   | Final exact migration (including the two-phase invitation definitions) and seed ran twice: 8 profiles, 5 categories, and 5 requests remained idempotent; selected RLS/RPC/reviewer/renewal/progress/audit checks completed. Not Supabase/PostgREST/pgTAP.                  |
| SQL parser check                  | Passed (supplemental), 2026-08-29   | SQLFluff PostgreSQL parsing exited 0 for the final migration and invitation-integrity test; this is syntax evidence only                                                                                                                                                   |
| `supabase db reset --local`       | Not recorded                        | Docker/Podman/local PostgreSQL was unavailable                                                                                                                                                                                                                             |
| `supabase test db`                | Environment blocked, 2026-08-29     | Attempt could not connect to `127.0.0.1:54322`; none of the 213 authored pgTAP assertions (six files; plans 54+51+32+8+20+48) is recorded as a native pass                                                                                                                 |
| `pnpm build`                      | Passed (shape only), 2026-08-29     | Production build completed with inert syntactically valid environment placeholders; no external connectivity was exercised. Node 20 emitted Supabase deprecation warnings; use Node 22.                                                                                    |
| Full `pnpm test:e2e`              | Not run                             | `tests/e2e/portal.spec.ts` exists, but its required local Supabase/Auth stack was unavailable                                                                                                                                                                              |
| Design-preview Playwright         | Passed (static fixture), 2026-08-29 | `tests/e2e/design-preview.spec.ts`: 2/2 Chromium tests passed after the destructive-contrast correction; desktop four-screen axe/overflow and mobile dashboard/form checks passed                                                                                          |
| `pnpm audit --audit-level=high`   | Passed, 2026-08-29                  | “No known vulnerabilities found” for the current `pnpm-lock.yaml`; rerun on the release commit                                                                                                                                                                             |
| GitHub Actions                    | Configured, run not observed        | `.github/workflows/ci.yml` contains application, database, and browser jobs; require a passing run from the exact release commit                                                                                                                                           |
| Manual role/authorization QA      | Not recorded                        | Test every persona and direct URL/ID substitution                                                                                                                                                                                                                          |
| Accessibility + 4 viewports       | Partial, 2026-08-29                 | Static fixtures: Playwright axe/overflow passed as above; interactive Chrome covered all four screens at 390×844, 768×1024, 1440×900, and 1920×1080 with no overflow/current console issue. Authenticated flows, 200% zoom, full keyboard order, and screen reader remain. |
